...
type NotificationType = 'alert' | 'school' | 'contract' | 'payment' | 'system';
...
const mockNotifications: AdminNotification[] = [
  {
    ...
    description: 'Colégio São Pedro - Fatura de junho vencida',
    origin: 'Colégio São Pedro',
  ...
  },
  {
    ...
    description: 'Escola Municipal Centro completou cadastro',
    origin: 'Sistema',
    timestamp: new Date(Date.now() - 5 * 60 * 60 * 1000),
    actionPath: '/admin/schools'
  },
  {
    id: '3',
    type: 'contract',
    ...
    description: 'Instituto Educacional Norte - 30 dias para renovação',
    origin: 'Instituto Educacional Norte',
    timestamp: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000),
    actionPath: '/admin/contracts'
  },
  {
    id: '4',
    type: 'alert',
    severity: 'high',
    status: 'unread',
    title: 'Múltiplas câmeras offline',
    description: 'Colégio Adventista reportou 5 câmeras offline',
    origin: 'Colégio Adventista',
  ...
  },
  {
    ...
    severity: 'low',
    status: 'read',
    title: 'Atualização de sistema concluída',
    description: 'Versão 2.4.1 instalada com sucesso',
    origin: 'Sistema',
  ...
  },
  {
    ...
    description: 'Colégio São Pedro - Fatura de maio quitada',
    origin: 'Colégio São Pedro',
...
export function AdminNotificationsScreen({ isFirstAccess = false }: AdminNotificationsScreenProps) {
  const [notifications, setNotifications] = useState<AdminNotification[]>(isFirstAccess ? [] : mockNotifications);
  ...
  const filteredNotifications = notifications
    ...
    .sort((a, b) => {
      ...
      const aIsCritical = (a.type === 'alert' || a.type === 'payment') && a.severity === 'high' && (now - a.timestamp.getTime()) < fiveMinutes;
      const bIsCritical = (b.type === 'alert' || b.type === 'payment') && b.severity === 'high' && (now - b.timestamp.getTime()) < fiveMinutes;
  ...
  const showSeverityFilter = filterType === 'alert' || filterType === 'payment' || filterType === 'school' || filterType === 'contract' || filterType === 'all';
    ...
    return (
      <div className="space-y-6">
        <EmptyState
          ...
          description="As notificações do sistema aparecerão aqui quando houver atividades importantes, alertas ou atualizações das escolas."
        />
      </div>
    );
  }
              ...
              {selectedIds.size > 0 && (
                <>
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      setNotifications(notifications.map(n => 
                        selectedIds.has(n.id) ? { ...n, status: 'read' as NotificationStatus } : n
                      ));
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {filteredNotifications.map((notification) => {
                      const isCritical = (notification.type === 'alert' || notification.type === 'payment') && 
            ...
            <div className="space-y-3">
              {filteredNotifications.map((notification) => {
                ...
                return (
                  ...
                  >
                    <div className="space-y-3">
                      {/* Header with badges (checkbox hidden on mobile) */}
